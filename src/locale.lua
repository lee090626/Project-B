...
pair("run_end.status.title.time", "HUNT COMPLETE", "사냥 종료")
pair("run_end.status.title.boss_failed", "BOSS TRIAL FAILED", "보스전 실패")
pair("run_end.status.title.victory", "FINAL VICTORY", "최종 승리")
pair("run_end.status.title.unknown", "RUN COMPLETE", "런 완료")
pair("run_end.status.subtitle.time", "The hunt closes. Spend the growth before the next strike.", "사냥이 끝났습니다. 다음 돌입 전에 성장을 정비하세요")
pair("run_end.status.subtitle.boss_failed", "The final foe holds for now. Strengthen the dragon and return.", "최종 적이 버텼습니다. 용을 키워 다시 도전하세요")
pair("run_end.status.subtitle.victory", "The baby dragon has devoured the final challenge.", "아기 용이 마지막 시련을 먹어치웠습니다")
pair("run_end.status.subtitle.unknown", "The run has settled into growth.", "이번 런의 성과가 성장으로 전환됩니다")
pair("run_end.status.callout.victory", "Ending condition met. The final path is complete.", "엔딩 조건 달성. 마지막 길이 완성됐습니다")
pair("run_end.status.callout.new_map", "New hunting ground unlocked from this result.", "이번 성과로 새 사냥터가 열렸습니다")
pair("run_end.status.callout.best_improved", "Current map best improved to {stars}/3 stars.", "현재 맵 최고 기록이 별 {stars}/3개로 상승했습니다")
pair("run_end.status.callout.stars_secured", "{stars}/3 stars secured for this hunt.", "이번 사냥에서 별 {stars}/3개를 확보했습니다")
pair("run_end.status.callout.no_stars", "No hunt stars earned. Invest growth and try a cleaner route.", "사냥 별을 얻지 못했습니다. 성장 후 더 깔끔한 경로를 노려보세요")
pair("run_end.status.stat.essence", "Essence", "에센스")
pair("run_end.status.stat.level", "Dragon Lv", "용 레벨")
pair("run_end.status.stat.evolution", "Evolution", "진화")
pair("run_end.status.stat.total_stars", "Total Stars", "총 별")
pair("run_end.status.next.map", "Next Map: {map}", "다음 맵: {map}")
pair("run_end.status.next.all_maps", "Map Progress", "맵 진행")
