...
function RunEndResultRenderer.draw(state, fonts)
    ...
    drawStarLine(state, x, y + 116, w, "run_end.result.run_stars", { stars = state.runStarsEarned or 0 })
    drawStarLine(state, x, y + 146, w, "run_end.result.map_best", { best = currentMapBest })
    drawStarLine(state, x, y + 176, w, "run_end.result.total_stars", { stars = totalStars, max = maxStars })
    drawResultLine(state, x, y + 206, w, "run_end.result.current_essence", { essence = state.meta and state.meta.essence or 0 })
    drawResultLine(state, x, y + 236, w, "run_end.result.level", { level = state.nestProgress.level })
    love.graphics.printf(nextUnlockText, x + 36, y + 266, w - 72, "left")
    drawResultLine(state, x, y + 296, w, "run_end.result.evolution", { evolution = ref(state.nestProgress.evolutionKey) })
