local PresentationConfig = {
    uiIcons = {
        essence = {
            path = "assets/ui/icons/essence.png",
            hudSize = 24,
            chipSize = 32,
            rewardSize = 48,
            largeSize = 64,
        },
    },
    runChoiceUi = {
        cardWidth = 272,
        cardHeight = 360,
        cardGap = 28,
        cardFramePaths = {
            common = "assets/ui/cards/run_choice_common.png",
            rare = "assets/ui/cards/run_choice_rare.png",
            mythic = "assets/ui/cards/run_choice_mythic.png",
        },
    },
    runEndTreeUi = {
        worldScale = 460,
        minZoom = 0.4,
        maxZoom = 2.6,
        zoomStepUp = 1.12,
        zoomStepDown = 0.88,
        nodeRadius = 24,
        clickThreshold = 8,
        tabWidth = 140,
        tabHeight = 34,
        tabGap = 12,
    },
    runEndResultUi = {
        marginX = 30,
        marginY = 28,
        maxWidth = 860,
        maxHeight = 468,
        padX = 44,
        titleY = 24,
        subtitleY = 62,
        starCenterY = 152,
        starRadius = 28,
        starGap = 78,
        calloutY = 200,
        calloutHeight = 42,
        statsY = 272,
        statGap = 14,
        nextY = 354,
        progressHeight = 14,
        continueBottom = 34,
        themes = {
            time = {
                overlay = { 0.0, 0.0, 0.0, 0.74 },
                panelFill = { 0.05, 0.07, 0.09, 0.97 },
                panelInner = { 0.09, 0.12, 0.14, 0.9 },
                panelLine = { 0.87, 0.73, 0.42, 0.96 },
                panelGlow = { 0.22, 0.55, 0.7, 0.24 },
                accent = { 0.95, 0.78, 0.42, 1.0 },
                accentSoft = { 0.52, 0.8, 0.84, 1.0 },
                text = { 0.98, 0.94, 0.84, 1.0 },
                dim = { 0.72, 0.84, 0.86, 1.0 },
                calloutFill = { 0.14, 0.12, 0.08, 0.94 },
                calloutLine = { 0.95, 0.78, 0.42, 0.98 },
                starFill = { 0.22, 0.16, 0.08, 0.98 },
                starLine = { 1.0, 0.82, 0.34, 1.0 },
                emptyStarFill = { 0.06, 0.08, 0.1, 0.95 },
                emptyStarLine = { 0.36, 0.43, 0.48, 0.95 },
                progressBg = { 0.08, 0.1, 0.11, 0.94 },
                progressFill = { 0.35, 0.76, 0.72, 0.96 },
            },
            boss_failed = {
                overlay = { 0.0, 0.0, 0.0, 0.76 },
                panelFill = { 0.08, 0.05, 0.06, 0.97 },
                panelInner = { 0.14, 0.08, 0.08, 0.9 },
                panelLine = { 0.93, 0.46, 0.3, 0.97 },
                panelGlow = { 0.68, 0.16, 0.12, 0.26 },
                accent = { 0.96, 0.52, 0.28, 1.0 },
                accentSoft = { 0.94, 0.72, 0.48, 1.0 },
                text = { 0.98, 0.92, 0.84, 1.0 },
                dim = { 0.88, 0.72, 0.68, 1.0 },
                calloutFill = { 0.18, 0.08, 0.07, 0.94 },
                calloutLine = { 0.96, 0.5, 0.28, 0.98 },
                starFill = { 0.24, 0.12, 0.08, 0.98 },
                starLine = { 1.0, 0.72, 0.36, 1.0 },
                emptyStarFill = { 0.08, 0.06, 0.06, 0.95 },
                emptyStarLine = { 0.44, 0.3, 0.28, 0.95 },
                progressBg = { 0.11, 0.07, 0.07, 0.94 },
                progressFill = { 0.92, 0.48, 0.3, 0.96 },
            },
            victory = {
                overlay = { 0.0, 0.0, 0.0, 0.72 },
                panelFill = { 0.04, 0.08, 0.07, 0.97 },
                panelInner = { 0.08, 0.13, 0.11, 0.9 },
                panelLine = { 0.96, 0.82, 0.42, 0.98 },
                panelGlow = { 0.35, 0.9, 0.64, 0.28 },
                accent = { 1.0, 0.84, 0.38, 1.0 },
                accentSoft = { 0.42, 0.9, 0.66, 1.0 },
                text = { 0.98, 0.96, 0.86, 1.0 },
                dim = { 0.75, 0.9, 0.82, 1.0 },
                calloutFill = { 0.1, 0.16, 0.1, 0.94 },
                calloutLine = { 0.98, 0.82, 0.42, 0.98 },
                starFill = { 0.24, 0.18, 0.08, 0.98 },
                starLine = { 1.0, 0.86, 0.34, 1.0 },
                emptyStarFill = { 0.06, 0.09, 0.08, 0.95 },
                emptyStarLine = { 0.36, 0.5, 0.42, 0.95 },
                progressBg = { 0.08, 0.12, 0.1, 0.94 },
                progressFill = { 0.42, 0.9, 0.66, 0.96 },
            },
        },
    },
    runHudUi = {
        padding = 14,
        topBarHeight = 74,
        groupGap = 10,
        chipPadX = 10,
        toastDuration = 2.2,
        autosaveDuration = 2.8,
    },
    guideUi = {
        width = 620,
        minHeight = 132,
        topYGame = 100,
        topYChoice = 24,
        topYRunEnd = 126,
        buttonWidth = 116,
        buttonHeight = 30,
        padX = 22,
        padY = 18,
    },
    hudTheme = {
        panelFill = { 0.04, 0.08, 0.08, 0.92 },
        panelInner = { 0.08, 0.12, 0.12, 0.82 },
        panelLine = { 0.87, 0.73, 0.42, 0.95 },
        panelGlow = { 0.23, 0.74, 0.58, 0.25 },
        text = { 0.97, 0.94, 0.84, 1.0 },
        dim = { 0.73, 0.86, 0.8, 1.0 },
        accent = { 0.92, 0.78, 0.44, 1.0 },
        accentSoft = { 0.35, 0.76, 0.6, 1.0 },
        warning = { 0.92, 0.47, 0.25, 1.0 },
        danger = { 0.93, 0.36, 0.28, 1.0 },
        progressBg = { 0.1, 0.14, 0.14, 0.92 },
        progressFill = { 0.36, 0.88, 0.68, 0.95 },
        progressDone = { 0.93, 0.78, 0.42, 0.95 },
        buttonFill = { 0.12, 0.16, 0.14, 0.96 },
        buttonLine = { 0.92, 0.76, 0.43, 0.96 },
        buttonText = { 0.98, 0.94, 0.84, 1.0 },
        chipFill = { 0.08, 0.11, 0.11, 0.94 },
        chipLine = { 0.31, 0.75, 0.62, 0.95 },
    },
    helpTheme = {
        panelFill = { 0.08, 0.07, 0.06, 0.95 },
        panelInner = { 0.16, 0.13, 0.1, 0.9 },
        panelLine = { 0.86, 0.72, 0.42, 0.96 },
        text = { 0.97, 0.94, 0.86, 1.0 },
        dim = { 0.8, 0.84, 0.78, 1.0 },
        chipFill = { 0.14, 0.12, 0.1, 0.96 },
        chipLine = { 0.33, 0.77, 0.61, 0.96 },
    },
    guideTheme = {
        panelFill = { 0.07, 0.08, 0.11, 0.96 },
        panelInner = { 0.12, 0.14, 0.18, 0.92 },
        panelLine = { 0.93, 0.79, 0.44, 0.98 },
        panelGlow = { 0.32, 0.8, 0.65, 0.28 },
        title = { 0.98, 0.95, 0.85, 1.0 },
        text = { 0.86, 0.93, 0.95, 1.0 },
        dim = { 0.74, 0.82, 0.88, 1.0 },
        chipFill = { 0.08, 0.1, 0.12, 0.96 },
        chipLine = { 0.38, 0.82, 0.7, 0.98 },
        buttonFill = { 0.14, 0.18, 0.24, 0.98 },
        buttonLine = { 0.93, 0.79, 0.44, 0.98 },
    },
    runChoiceTheme = {
        overlay = { 0.01, 0.03, 0.03, 0.78 },
        title = { 0.97, 0.92, 0.78, 1.0 },
        summary = { 0.75, 0.87, 0.83, 1.0 },
        common = {
            fill = { 0.08, 0.11, 0.11, 0.96 },
            inner = { 0.12, 0.16, 0.16, 0.94 },
            line = { 0.44, 0.84, 0.67, 0.96 },
            ribbon = { 0.2, 0.45, 0.38, 0.96 },
        },
        rare = {
            fill = { 0.07, 0.1, 0.12, 0.96 },
            inner = { 0.11, 0.16, 0.18, 0.94 },
            line = { 0.49, 0.86, 0.92, 0.96 },
            ribbon = { 0.2, 0.41, 0.52, 0.96 },
        },
        mythic = {
            fill = { 0.12, 0.08, 0.06, 0.97 },
            inner = { 0.18, 0.12, 0.09, 0.95 },
            line = { 0.96, 0.78, 0.43, 0.98 },
            ribbon = { 0.64, 0.32, 0.12, 0.98 },
        },
    },
}

return PresentationConfig
